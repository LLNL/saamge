...
/**
   ...
   The limitation of SharingMap is that it is all based on Hypre
   matrix multiplies, so you cannot do reductions where the operation
   is something other than addition.
   
   In particular, for the spectral method we want to do reduction
   where the operation is some kind of SVD, which requires something
   more complicated.

   The complicated logic on the Reduce side is because we are leaving
   the actual reduce operation to the user, so you can think of it
   more as a "collect" operation onto the master, where the user is
   responsible to do what is necessary.

   This is "fairly" generic but not completely, if you want to use for
   a datatype other than mfem::DenseMatrix or mfem::Vector you need to
...
// this construction might be simpler if we used SharingMap instead of hypre_ParCSRMatrix
template <class T>
SharedEntityCommunication<T>::SharedEntityCommunication(
    MPI_Comm comm,
    hypre_ParCSRMatrix * entity_trueentity) 
    ...
    entity_trueentity(entity_trueentity)
{
    ...
    if (!comm_pkg) // we need to directly access the comm_pkg, so insure it has been built
    {
    ...
    int num_entities = entity_trueentity->row_starts[1] - entity_trueentity->row_starts[0];
        ...
        if (ete_offd_I[entity+1] - ete_offd_I[entity] == 1)
        {
            ...
            for (int recv=0; recv<comm_pkg->num_recvs; ++recv)
            {
                     ...
                     ++k)
                {
                    if (k == col)
                    {
                        // next line is really unnecessary, we never touch the unowned rows of entity_proc
                        entity_proc->AddAColumnInRow(entity); 
...
void SharedEntityCommunication<T>::BroadcastSizes(T ** mats)
{
    ...
    for (int j = 0; j < ete_offd->num_cols; ++j)
    {
        int owner = entity_master[ete_offdT_J[j]];
        MPI_Irecv(
    ...
    for (int j = 0; j < count_master_entities; ++j)
    {
        ...
        for (int neighbor=0; neighbor<neighbor_row_size; ++neighbor)
        {
            if (neighbor_row[neighbor] != comm_rank)
            {
                ...
                MPI_Isend(
                    &(send_headers[send_counter * header_length]), header_length,
