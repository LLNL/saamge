...
void nonconf_ip_first_coarse_schur_matrices(interp_data_t& interp_data,
                  ...
                  double theta, double delta, bool onlytheblocks=false, bool full_space=false)
{
    ...
    if (onlytheblocks)
    {
        interp_data.Aii = new DenseMatrix*[agg_part_rels.nparts];
        interp_data.Abb = new DenseMatrix*[agg_part_rels.nparts];
        interp_data.Aib = new DenseMatrix*[agg_part_rels.nparts];
    } else
    {
...
/**
    For testing. Assembles global coarse interior penalty matrix.

    The returned global matrix must be freed by the caller.
*/
static inline
HypreParMatrix *nonconf_ip_assemble_coarse_matrix(const interp_data_t& interp_data,
                    const agg_partitioning_relations_t& agg_part_rels,
                    const HypreParMatrix& cDof_TruecDof)
{
    ...
    for (int i=0; i < interp_data.nparts; ++i)
    {
        const DenseMatrix &Aii = *interp_data.Aii[i];
        const DenseMatrix &Abb = *interp_data.Abb[i];
        const DenseMatrix &Aib = *interp_data.Aib[i];
        SA_ASSERT(&Aii);
        SA_ASSERT(&Abb);
        SA_ASSERT(&Aib);
                               ...
                               bool schur, bool full_space)
{
    ...
    nonconf_ip_first_coarse_schur_matrices(*tg_data.interp_data, agg_part_rels, evals, theta, delta, !schur, full_space);
...
//                                                 tg_data.ltent_restr);
    ...
    if (schur)
        ...
        tg_data.Ac = nonconf_ip_assemble_coarse_matrix(*tg_data.interp_data, agg_part_rels,
                        *agg_part_rels.cface_cDof_TruecDof);
